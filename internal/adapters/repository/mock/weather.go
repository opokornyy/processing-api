...
func (m *MockWeatherRepository) seedData() {
	...
	m.Save(berlinWeather)
}

// Save stores weather data in memory
func (m *MockWeatherRepository) Save(weatherData *weather.WeatherData) error {
	if weatherData == nil {
...
func (m *MockWeatherRepository) GetLatestByLocation(location weather.Location) (*weather.WeatherData, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
